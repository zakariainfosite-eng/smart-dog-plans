-- Enhance operational cases: checkpoint link, observations, attachments.
...
BEGIN
  IF EXISTS (
    ...
    WHERE table_schema = 'public'
      AND table_name = 'operational_cases'
...
ALTER TABLE public.operational_cases
  DROP CONSTRAINT IF EXISTS operational_cases_notes_length_check;

ALTER TABLE public.operational_cases
  DROP CONSTRAINT IF EXISTS operational_cases_observations_length_check;

ALTER TABLE public.operational_cases
  ADD CONSTRAINT operational_cases_observations_length_check
  CHECK (observations IS NULL OR char_length(observations) <= 1000);

CREATE INDEX IF NOT EXISTS idx_operational_cases_checkpoint
  ON public.operational_cases(checkpoint_id);
...
CREATE INDEX IF NOT EXISTS idx_operational_case_attachments_case
  ON public.operational_case_attachments(case_id);
